...
imputeKNN <- function(obj, rowmax = 0.5, colmax = 0.8, k = 10,
  ...
  if (!is.beta) {
    if (assay == "array") {
      ...
      imputed.data <- impute.knn(assays(obj.clean)$Beta,
        k = k,
      ...
      imputed.data <- impute.knn(assays(obj.clean)$counts,
        k = k,
        rowmax = rowmax, colmax = colmax,
        maxp = maxp
      )$data
    ...
    if (assay == "array") {
      ...
      impute.knn(flogit(assays(obj.clean)$Beta),
        k = k,
        rowmax = rowmax, colmax = colmax,
        maxp = maxp
      )$data
      ...
      impute.knn(flogit(assays(obj.clean)$counts),
        k = k,
        rowmax = rowmax, colmax = colmax,
        maxp = maxp
      )$data
