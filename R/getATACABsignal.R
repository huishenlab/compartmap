...
getATACABsignal <- function(obj, res = 1e6, parallel = FALSE, chr = NULL,
  ...
  atacCompartments <- function(obj, original.obj, res = 1e6, chr = NULL, targets = NULL,
    ...
    obj.bins <- shrinkBins(obj, original.obj,
      prior.means = prior.means, chr = chr,
    ...
    obj.bootstrap <- bootstrapCompartments(obj, original.obj,
  ...
  if (parallel & isFALSE(group)) {
    atac.compartments <- mclapply(columns, function(s) {
      ...
      sort(unlist(as(lapply(chr, function(c) {
        atacCompartments(obj.sub, obj,
          res = res,
  ...
  if (parallel & isTRUE(group)) {
    atac.compartments <- sort(unlist(as(
      mclapply(chr, function(c) {
        atacCompartments(obj, obj,
          res = res,
          chr = c, targets = targets, genome = genome,
